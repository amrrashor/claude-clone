    ...
    return (
        showModal && (
            <div className="bg-[rgba(0,0,0,0.6)] absolute top-0 left-0 right-0 bottom-0 z-50 flex justify-center items-center">
                <motion.div 
                    className="p-4 w-8/12 lg:w-4/12 mx-auto bg-[#1a1918] rounded-xl shadow-2xl"
                    initial={{opacity: 0, translateY:10}}
                    animate={{opacity: 1, translateY:0}}
                    exit={{opacity: 0, translateY:10}}
                    transition={{duration:0.3}}
                >
                    <h1 className="text-lg md:text-xl text-white font-bold mb-3 text-left">{modalTitle}</h1>
                    {isRename ? (
                        <input 
                            type="text" 
                    ...
                    <div className="flex justify-end items-end">
                        <button onClick={() => setShowModal(false)} className="mr-4 bg-[#323131] rounded-lg px-2 py-1 text-xs md:text-[16px] text-white cursor-pointer font-bold">
                            Cancel
                        </button>
                        <button onClick={isRename ? handleTitleChange : handleChatDelete} className={`${isRename ? "bg-[#da7756]" : "bg-amber-600"}  text-xs rounded-lg px-2 py-1 md:text-[16px] text-white font-bold cursor-pointer`}>
