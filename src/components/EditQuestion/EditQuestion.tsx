...
interface props {
    ...
    reGenerateAction?:any
}
const EditQuestion = ({reGenerateAction, onSave, setEditingIndex, oldTitle,  setShowInputField, action}:props) => {
        ...
        <div>
            ...
            <div className='flex items-center'>
                ...
                <div className='flex items-center'>
                    ...
                    <button onClick={() => {dispatch(reGenerateAction(true)); dispatch(action(text)); setShowInputField(false); setEditingIndex(null);   onSave(text);}} className={`bg-[#da7756] rounded-lg px-4 py-1 text-[16px] text-white font-bold cursor-pointer`}>Save</button>
                </div>
            </div>
        </div>
    )
